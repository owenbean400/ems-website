...
If enabled truck entry within setting windows, a truck entry will display after mileage input.
Tap on the selection box to choose the truck that was used during rescue.
When finished, hit `Complete` to finish the rescue time.

![Truck Entry Screen](../../../images/doc/screen/time_truck_screen.jpg)

## Notes

If enabled notes within setting windows, a note entry will display after the mileage input or truck entry.
Enter a note that you want to add with the rescue time.
When finished, hit `Complete` to finish the rescue time.

![Notes Entry Screen](../../../images/doc/screen/notes_screen.jpg)
